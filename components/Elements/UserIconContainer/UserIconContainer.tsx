...
const UserIconContainer = ({ userIconTitle }: { userIconTitle: string }) => {
  return (
    <div
      className={`group flex gap-4 h-10 transition-colors bg-transparent dark:text-[${colorPallette.darkText}] ease-linear duration-100 group items-center rounded-md flex-1 overflow-hidden px-[5px] justify-center max-w-[8rem] hover:bg-accent dark:hover:bg-accent hover:cursor-pointer`}>
      <Image
        src={image}
        alt="image "
