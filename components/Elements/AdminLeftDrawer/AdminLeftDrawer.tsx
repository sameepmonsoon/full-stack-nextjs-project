...
const AdminLeftDrawer = () => {
  ...
  return (
    <>
      <div
        className={`fixed top-0 z-[99] bg-black/30 w-screen h-screen lg:hidden ${
          siderState === 2
      ...
      <nav
        id="adminLeftDrawer"
        className={`adminLeftDrawer fixed bg-transparent dark:text-white px-5 overflow-y-scroll h-screen visible transition-width duration-300 ease-in-out ${
          siderState == 0
      ...
      >
        ...
        {adminLeftDrawerList.map((item: any, index: number) => (
          <>
            <ul
              key={index}
              className={`w-full flex flex-col items-start justify-start gap-2 bg-transparent dark:text-gray-300 dark:border-b-gray-400/40  ${
                hideSiderDetail
